...
EXPECTED_BT_PROFILE = {
    ...
    "task_complete_gate": "make_sandwich.task_complete",
...
EXPECTED_TASK_READMES = {
    ...
    "lerobot_bt_lunch_table_bussing_README.md": {
...
def _runtime_dir() -> Path:
    return Path(__file__).resolve().parents[2] / "src" / "lerobot_bt_runtime_cpp"
...
def _python_dir() -> Path:
    return Path(__file__).resolve().parents[2] / "src" / "lerobot_bt_python"
...
def test_make_sandwich_tree_matches_current_task() -> None:
    tree_path = _runtime_dir() / "trees" / "make_sandwich.xml"
...
def test_make_sandwich_executor_profile_covers_xml_skill_names() -> None:
    profile_path = _python_dir() / "make_sandwich_executor.yaml"
...
def test_launch_files_cover_all_runtime_profiles() -> None:
    ...
    for launch_name, (tree_name, profile_name) in EXPECTED_LAUNCH_FILES.items():
        ...
        assert "lerobot_bt_runner" in launch_text
...
def test_make_sandwich_profile_matches_current_robot_task() -> None:
    profile_path = _runtime_dir() / "config" / "make_sandwich_bt.yaml"

    assert _load_simple_bt_profile(profile_path) == EXPECTED_BT_PROFILE


def test_make_sandwich_tree_placeholders_are_defined_in_profile() -> None:
    tree_path = _runtime_dir() / "trees" / "make_sandwich.xml"
    profile_keys = set(_load_simple_bt_profile(_runtime_dir() / "config" / "make_sandwich_bt.yaml"))
    root = ElementTree.parse(tree_path).getroot()
