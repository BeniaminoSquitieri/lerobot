...
BLOCKS: list[tuple[str, str]] = [
    ...
    (
        "subtitle",
        f"Pipeline finale 'make coffee': percezione, spatial-prior gate, "
    ...
    (
        "bullet",
        "Percezione RGB-D (nodo ROS 2): rileva gli oggetti e ne stima la "
        "posa 3D nel frame base_link. E' l'UNICA fonte di verita' sulla "
        "posizione degli oggetti.",
    ),
    (
        "bullet",
        "Spatial-prior gate (deterministico): PRIMA di lanciare una skill, "
    ...
    (
        "bullet",
        "Skill ACT (policy neurale): esegue il gesto vero e proprio "
        "(prendere la capsula e inserirla, chiudere la macchina).",
    ),
    (
        "bullet",
        "VLM verifier (Qwen3-VL): DOPO l'esecuzione valuta semanticamente "
    ...
    (
        ...
        "parse_object_pose_json in perception/spatial_prior_gate.py, invece, accettava la "
    ...
    (
        ...
        "src/lerobot_bt_python/perception/spatial_prior_gate.py (repo lerobot).",
    ),
    (
        "code",
    ...
    (
        "body",
        "File: src/tests/lerobot_bt/test_perception_spatial_prior.py. Tre nuovi test: "
    ...
    (
        "bullet",
        "mu = [0.68432, -0.26041, 0.15019] m (media della posizione "
        "end-effector all'istante di presa, usata come PROXY della posa "
        "oggetto).",
    ),
    ("bullet", "mahalanobis_threshold = 3.36821 (soglia a confidence_level 0.99 su 3 gradi di liberta')."),
    (
        "bullet",
        "sigma = matrice 3x3 di covarianza; translation_std_m ~ [0.026, "
    ...
    (
        "bullet",
        "shadow: il gate calcola e logga il verdetto ma NON blocca la skill "
    ...
    ("body", "Ogni decisione emette una riga di log greppabile, ad esempio:"),
    (
        "code",
    ...
    (
        "bullet",
        "solve_rigid_transform(camera, base): algoritmo di Kabsch/Umeyama; "
        "restituisce R (rotazione 3x3), t (traslazione) e rms (errore "
        "residuo).",
    ),
    (
        "bullet",
        "rotation_matrix_to_quaternion_xyzw(matrix): converte R in "
        "quaternione [x, y, z, w] (formato usato dal TF).",
    ),
    (
        "bullet",
        "_format_yaml_block(...): stampa direttamente il blocco "
        "'camera_static_tf_map:' pronto da incollare nel config.",
    ),
    (
        "bullet",
        "main(): legge un JSON {camera_name, parent_frame_id, "
    ...
    (
        "bullet",
        "vlm_live/prompt.py: nuova format_scene_context(scene_facts_json) che "
    ...
    (
        "bullet",
        "build_prompt aggiunge il blocco 'Perception scene facts (...)' SOLO "
        "se request contiene 'scene_context'; senza, nessuna regressione.",
    ),
    (
        "bullet",
        "vlm_live/node.py: importa format_scene_context; in _run_vlm calcola "
    ...
    (
        "bullet",
        "Skill ACT 'pick_and_insert_capsule' -> HSP-IIT/act_put_coffee. "
    ...
    (
        "bullet",
        "Percezione (segmentazione zero-shot) -> OWL-ViT "
    ...
    (
        "bullet",
        "VLM verifier -> Qwen3-VL-32B-Instruct. Modello vision-language che "
    ...
    (
        "bullet",
        "Prior spaziale gaussiano (mu, sigma) fittato da 50 dimostrazioni "
        "con perception/fit_spatial_prior.py. Non e' una rete neurale: e' un modello "
    ...
    (
        "bullet",
        "panda_live_viewer/run.sh: il vecchio blocco di export inline e' "
        "stato sostituito da 'source \"$SCRIPT_DIR/ros_env.sh\"' (DRY).",
    ),
    (
        "bullet",
        "lerobot/scripts/run_demo_bt_trial.sh: aggiunto 'source "
        '"$REPO_ROOT/ros_env.sh"\' subito dopo il calcolo di REPO_ROOT.',
    ),
    (
        "bullet",
        "~/.bashrc (locale): aggiunto un blocco delimitato '>>> "
    ...
    ("code", "source ~/lerobot/ros_env.sh        # oppure il percorso del repo sul server"),
    ("body", "Da quel momento ogni nuova shell sul server e' configurata, senza piu' export manuali."),
    ("h1", "11. Documentazione aggiornata e nota sull'ambiente Python"),
    (
        "bullet",
        "docs/spatial_prior_gating.md: conteggio test 27->30; nuove sezioni "
        "10 (calibrazione hand-eye / camera_static_tf_map), 11 "
        "(arricchimento scene-facts VLM one-way), 12 (smoke test offline).",
    ),
    (
        "bullet",
        "docs/DEMO_FINALE_COMMANDS.md: aggiornati i passi 11c.2 (smoke "
        "script + 30 test + avviso conda), 11c.5 (calibrazione hand-eye), "
        "11b.8 (nota scene_facts del VLM).",
    ),
    (
        "bullet",
        "panda_live_viewer/README.md: note su arricchimento scene_facts in "
    ...
    ("bullet", "Modificato: src/lerobot_bt_python/perception/spatial_prior_gate.py (parser dict+list)."),
    ("bullet", "Modificato: tests/lerobot_bt/test_perception_spatial_prior.py (+3 test, 30 totali)."),
    (
        "bullet",
        "Modificato: src/lerobot_bt_python/make_coffee_executor.yaml "
    ...
    ("bullet", "Modificato: docs/spatial_prior_gating.md, docs/DEMO_FINALE_COMMANDS.md."),
    ...
    (
        "body",
        "tests/lerobot_bt/test_perception_spatial_prior.py: 30 test PASS. "
