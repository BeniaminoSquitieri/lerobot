...
/**
 * @file wait_for_vlm_verdict_node.hpp
 * @brief Declares the BehaviorTree.CPP leaf that waits for VLM verdicts.
 *
 ...
 * XML instantiates this behavior as WaitForVLMVerdict with a `check_name`
 * input. The same node waits for both human/VLM gates and post-skill checks.
 */
...
/**
 * @brief BehaviorTree.CPP stateful action node for polling VLM check state.
 *
 ...
 */
class WaitForVLMVerdictNode : public BT::StatefulActionNode
{
public:
  /// ROS2 service type used to fetch the latest VLM check state.
  using ServiceT = lerobot_bt_interfaces::srv::GetSkillVerification;
   ...
   * @return A single required check_name input port.
   */
  static BT::PortsList providedPorts();

  /**
   * @brief Reads the check name and starts the first VLM state request.
