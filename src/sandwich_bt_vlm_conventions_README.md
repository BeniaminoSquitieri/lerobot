...
This page defines the shared contract used by the sandwich BT stack and by the
...
VLM/manual verifier reports a verdict on /sandwich_bt/vlm_result
...
- `skill_name`: the gate or skill name printed on `/sandwich_bt/vlm_request`.
- `attempt_id`: the attempt id printed on `/sandwich_bt/vlm_request`; `0` means
