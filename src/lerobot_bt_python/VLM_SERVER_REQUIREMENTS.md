...
È pensato per essere dato a un altro agente AI che deve implementare il lato
...
⚠️ I nomi dei topic sono configurabili nell'executor YAML (campi
...
| `task` | `string` | **Descrizione testuale del task** (es. `"Pick the toast upon the table."`). Campo aggiunto di recente: usalo nel prompt per dare contesto semantico al VLM. Può essere stringa vuota per i VLM gate. |
...
questi campi. Ci sono DUE modalità di risposta:

### Modalità A: Risposta con `status` (semplice, consigliata)
...
### Modalità B: Risposta con `next_action` (per flussi più complessi)
...
|-------|------|-------------|-------------|
| `skill_name` | `string` | **SÌ** | **Deve coincidere esattamente** con quello ricevuto nella richiesta |
| `attempt_id` | `int` | **SÌ** | **Deve coincidere** con l'`attempt_id` della richiesta attiva. Usare `0` per applicare al più recente |
| `status` | `string` | SÌ* | Uno dei valori in `allowed_statuses` (vedi §4) |
| `next_action` | `string` | SÌ* | Alternativa a `status`, uno dei valori in `allowed_next_actions` (vedi §4) |
...
|--------|---------------|---------------|
| `PENDING` | Il BT **aspetta** (RUNNING). Il check non è ancora partito. | Subito dopo aver ricevuto la richiesta, mentre il VLM sta caricando il modello |
| `RUNNING` | Il BT **aspetta** (RUNNING). Il VLM sta processando le immagini. | Durante l'inferenza VLM |
| `WAIT_HUMAN` | Il BT **aspetta** (RUNNING). Serve un intervento umano. | Quando il VLM non può decidere e serve un operatore |
...
| `SUCCESS` | Il BT **avanza** al nodo successivo. | La scena è corretta, il task è completato |
| `FAILURE` | Il BT **ritenta** la skill (via `RetryUntilSuccessful`). | La scena non è corretta, il robot deve riprovare |
...
- Se l'`attempt_id` nella risposta non corrisponde più all'ultimo tentativo
  attivo, la risposta viene **scartata silenziosamente** (il robot ha già
  ricominciato).
...
- Se arriva un verdetto VLM (`SUCCESS`, `FAILURE`, `WAIT_HUMAN`, o
  `MANUAL_INTERVENTION_REQUIRED`) **mentre una policy è ancora in esecuzione**
...
Il server VLM **può** pubblicare aggiornamenti intermedi per indicare che sta
lavorando:
...
// Se il VLM rileva che serve aiuto umano
...
### Opzione B: Chiamata diretta al robot (solo se rodabile)
...
comunicazione è semplificata a stringhe semplici:
...
Il formato della richiesta è un template configurabile (parametro
...
- Consigliato: usare un `MultiThreadedExecutor` se il VLM è bloccante
...
- **Idempotenza**: pubblicare lo stesso risultato più volte non deve
...
- Usare un meccanismo di cancellazione: se arriva una nuova richiesta per
  la stessa skill con `attempt_id` più alto, abortire l'inferenza precedente
...
- [ ] Legge correttamente il JSON in arrivo (campi: `skill_name`, `attempt_id`, `status`, `message`, `task`)
- [ ] Usa il campo `task` (quando non vuoto) nel prompt VLM per dare contesto semantico
...
- [ ] Pubblica `RUNNING` (o `PENDING`) subito dopo aver ricevuto la richiesta, se l'inferenza è lenta
- [ ] Quando arriva una nuova richiesta con `attempt_id` più alto, abortisce l'inferenza in corso
- [ ] Non crasha se riceve JSON malformato
- [ ] Non crasha se `skill_name` è vuoto
