...
# Bootstrap script for LeRobot BT stack on fresh machines.
# Expected usage:
#   1) conda activate <your_env>
#   2) bash useful_scripts/bootstrap_lerobot_bt.sh
...
usage() {
  cat <<'EOF'
Usage:
  bash useful_scripts/bootstrap_lerobot_bt.sh [options]
...
cat > "${CONDA_PREFIX}/etc/conda/activate.d/lerobot_bt_env.sh" <<'EOF'
#!/usr/bin/env bash
export PKG_CONFIG_PATH="$CONDA_PREFIX/lib/pkgconfig:$CONDA_PREFIX/share/pkgconfig:${PKG_CONFIG_PATH:-}"
export CMAKE_PREFIX_PATH="$CONDA_PREFIX:${CMAKE_PREFIX_PATH:-}"
EOF
chmod +x "${CONDA_PREFIX}/etc/conda/activate.d/lerobot_bt_env.sh"
...
# Ensure lerobot_bt_interfaces source exists or clone from user-provided remote
if [[ ! -d "${REPO_ROOT}/src/lerobot_bt_interfaces" ]]; then
  if [[ -n "${LEROBOT_BT_REMOTE:-}" ]]; then
    log "src/lerobot_bt_interfaces not found — cloning from LEROBOT_BT_REMOTE"
    git clone "${LEROBOT_BT_REMOTE}" "src/lerobot_bt_interfaces"
  else
    log "Warning: src/lerobot_bt_interfaces not present."
    log "If this repo is split, set LEROBOT_BT_REMOTE to the git URL and re-run."
    log "Example: LEROBOT_BT_REMOTE=https://github.com/<org>/lerobot_bt_interfaces.git bash $0"
  fi
fi
...
if [[ "${SKIP_PYTHON}" -eq 0 ]]; then
  ...
  if [[ "${FULL_PYTHON}" -eq 1 ]]; then
    ...
    log "Installing minimal Python dependencies for the LeRobot BT stack"
...
if [[ "${SKIP_BUILD}" -eq 0 ]]; then
  ...
  BUILD_PACKAGES+=(lerobot_bt_interfaces lerobot_bt_runtime_cpp)
  ...
  if [[ ${BUILD_STATUS} -ne 0 ]]; then
    ...
    colcon build --base-paths src --packages-select lerobot_bt_interfaces --event-handlers console_direct+ --cmake-args -DCMAKE_VERBOSE_MAKEFILE=ON 2>&1 | tee "${REPO_ROOT}/log/lerobot_bt_interfaces_build.log" || true
    log "Please inspect ${REPO_ROOT}/log/lerobot_bt_interfaces_build.log for details."
    die "Build failed. Common fixes: install 'empy' (python -m pip install empy), ensure ROS/colcon tools available, or set LEROBOT_BT_REMOTE and re-run."
  fi
fi
...
PREFIX_PATHS=(
  "${REPO_ROOT}/install/lerobot_bt_runtime_cpp"
...
log "  ros2 pkg list | grep lerobot_bt"
log "  ros2 run lerobot_bt_runtime_cpp lerobot_bt_runner"
